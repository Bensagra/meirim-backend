...
// ---- Roles ----
// Cada usuario tiene un ARRAY de roles. Todos son MIEMBRO; además pueden tener
// EDITOR_FOTOS / ADMIN_ACTIVIDADES / SUPER_ADMIN (en cualquier combinación).
export const ROLES = ["MIEMBRO", "EDITOR_FOTOS", "ADMIN_ACTIVIDADES", "SUPER_ADMIN"];

/** Devuelve el array de roles de un usuario (siempre incluye MIEMBRO). */
export function rolesOf(user) {
  const list = Array.isArray(user?.roles) ? user.roles.slice() : [];
  if (!list.includes("MIEMBRO")) list.unshift("MIEMBRO");
  return list;
}

/** ¿El usuario tiene alguno de los roles pedidos? SUPER_ADMIN siempre pasa. */
...
export function requireRole(...allowed) {
  return async (req, res, next) => {
    await requireAuth(req, res, () => {
      if (userHasRole(req.user, ...allowed)) return next();
